...
class Kiwoom(QAxWidget):
    ...
    def tr_slot(self, sScrNo, sRQName, sTrCode, sRecordName, sPrevNext):
        if sRQName == "예수금상세현황요청":
            ...
            for i in range(cnt):
                ...
                output_1 = self.dynamicCall(
                "GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i, "외인순매수수량")
                output_2 = self.dynamicCall(
                "GetCommData(QString, QString, int, QString)", sTrCode, sRQName, i, "기관순매수수량")
                calculator_list.append(output_1)
                calculator_list.append(output_2)
    ...
    def get_cummulative_buy_or_sale_of_investor_wise(self, stock_code = None):
        ...
        self.dynamicCall("CommRqData(QString, QString, int, QString)",
                         "ETF일자별체결요청", "opt40008", 0, self.screen_calculation_stock)
